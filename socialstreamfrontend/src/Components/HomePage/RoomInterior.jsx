...
const RoomInterior = ({ roomId, onLeave }) => {
  ...
  const handlePlayVideo = async (video) => {
    try {
      // Fetch full video details to ensure we have all media URLs
      const videoService = (await import('../../services/videoService.js')).default;
      const fullVideo = await videoService.getVideoById(video.id);
      
      console.log('Host playing video - Full video object:', fullVideo);
      
      setCurrentVideo(fullVideo);
      sendChangeVideo(roomId, video.id);
    } catch (error) {
      console.error('Failed to fetch full video details for playback:', error);
      // Fallback to the video object from queue if fetch fails
